...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['submissionId']) && isset($_POST['status'])) {
        ...
        if ($conn->query($sql) === TRUE) {
            echo "Approval status updated successfully<br>";
        } else {
            echo "Error updating approval status: " . $conn->error . "<br>";
        }
        ...
        if ($status === "approved") {
            ...
            if ($result->num_rows > 0) {
                ...
                if ($targetConn->query($sql) === TRUE) {
                    echo "Data moved to the target table successfully<br>";
                } else {
                    echo "Error moving data to the target table: " . $targetConn->error . "<br>";
                }
            } else {
                echo "No data found in submissions table for the given submissionId<br>";
            }
